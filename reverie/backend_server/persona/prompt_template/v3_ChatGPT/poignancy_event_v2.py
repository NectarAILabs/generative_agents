...
async def run_gpt_prompt_event_poignancy(
  ...
  # ChatGPT Plugin ===========================================================
  def func_clean_up(gpt_response: Poignancy, prompt=""):
  ...
  def func_validate(gpt_response, prompt=""):
    try:
      poignancy = func_clean_up(gpt_response, prompt)
  ...
  provider_parameter = openai_config.get("other_providers", {}).get("poignancy_event_provider", None)
  example = {"poignancy": 5}
  ...
  output = await ChatGPT_safe_generate_response(
    ...
    verbose=verbose,
    response_format_name="Poignancy",
