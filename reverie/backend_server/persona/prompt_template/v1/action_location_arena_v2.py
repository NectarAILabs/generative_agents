...
from ..gpt_structure import safe_generate_structured_response, safe_generate_response
...
async def run_gpt_prompt_action_arena(
  ...
  def __func_validate(gpt_response, prompt=""):
    ...
    accessible_arenas = accessible_arena_str.split(", ")
    if arena in accessible_arenas:
      return True
  ...
  provider_parameter = openai_config.get("other_providers", {}).get("action_location_arena_provider", None)
  ...
  output = await safe_generate_response(
    prompt,
    gpt_param,
    repeat = 5,
    ...
    verbose = False,
    response_format_name = "ActionLoc"
  )
