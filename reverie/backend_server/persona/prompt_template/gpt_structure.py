...
if openai_config["embeddings-client"] == "azure":
    embeddings_client = setup_client(
        "azure",
        {
...
def ChatGPT_request(prompt):
    ...
    try:
        completion = client.chat.completions.create(
            model=openai_config["model"], messages=[{"role": "user", "content": prompt}]
        )
        content = completion.choices[0].message.content
        print("Response content:", content)
...
def generate_prompt(curr_input, prompt_lib_file="", prompt_template_str=""):
    ...
    if prompt_lib_file:
        ...
        raise ValueError(
            "Either prompt_lib_file or prompt_template_str must be provided."
        )
...
def safe_generate_structured_response(
    ...
    if func_validate and func_clean_up:
        for i in range(repeat):
            curr_gpt_response = GPT_structured_request(
                prompt, gpt_parameter, response_format
            )
            try:
                if not isinstance(curr_gpt_response, str) and func_validate(
                    curr_gpt_response, prompt=prompt
